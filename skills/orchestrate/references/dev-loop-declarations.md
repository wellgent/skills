...
- **A `/verify` skill** - repo-specific runtime verification (how to launch, sign in, and drive the app); the gate calls it for any ticket with runtime surface. The shared mechanics are its managed `protocol.md`; the `setup-verify` setup path in `wellgent/skills` scaffolds the skill and refreshes the protocol.
- **An ephemeral dev-server script** - start/stop/status on an explicit port, so takes and gates own their servers and the off-loop port stays untouched; declared in the contract and scaffolded by the same `setup-verify` run.
