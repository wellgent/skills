...
- **A `/verify` skill** - repo-specific runtime verification (how to launch, sign in, and drive the app); the gate calls it for any ticket with runtime surface. The shared mechanics are read in place from the installed `verify-protocol` skill (`wellgent/skills`, pinned beside `orchestrate`); scaffold the skill from its `verify-reference.md`.
