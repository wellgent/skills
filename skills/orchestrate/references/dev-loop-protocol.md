...
- **Teardown** - stop loop-owned dev servers, browser sessions, and any background process a take left, via its recorded handle (contract stop command or process group); before removing a take worktree, verify no process still holds its path (`lsof -a -d '^mem' +D <worktree>`; the exclusion drops pnpm store hard-links mapped by builds in other checkouts) - a survivor runs on from the deleted path, burning quota and writing debris the next take gets blamed for.
...
- **Observations** - anything actionable (a tooling defect, a gate blind spot, a skill gap) becomes an issue labelled `needs-triage`; narrative context goes in the report's Observations section.
  ...
  When the machine's identity cannot write the destination tracker, file on the project tracker with an `[upstream:<home>]` title marker - the operator's cross-project board sweep picks those up; the marker makes local pooling protocol, not a miss.
