...
5. **Land and close**: verify every take commit message before pushing - each carries a plain, correct `#<N>` mention with no closing-keyword collision; amend any message where the mention is absent, wrong, or colliding (takes routinely omit it even when the prompt carries the rule, so the mention the record depends on is the one this check puts there) - then land the take branch onto the landing branch - a two-location sequence: rebase the take branch **in its worktree** if the landing branch has moved (git holds the branch checked out there), then fast-forward and push **from the main checkout** (from inside the take's worktree the ff-merge is a silent "Already up to date" no-op - HEAD *is* the take branch - and only the later push or ship failure would surface it). Comment what shipped (see The record) and close the issue - closing is what unblocks its dependents.
...
6. **Teardown**: stop any dev server or browser session the gate started, then any background process the take left, via its recorded handle (the contract's stop command, or the process group of a recorded pid) - a pattern-matched kill misses a parent sleeping between retries, so anything killed by pattern gets a re-check after a quiet interval to confirm nothing respawned.
   Before removing the take's worktree and branch, verify no process still holds the worktree path (`lsof -a -d '^mem' +D <worktree>`) - a survivor runs on from the deleted path, burning quota and writing debris the next take gets blamed for.
   The `^mem` exclusion matters on pnpm projects: `node_modules` is hard-linked into the store, so a build in another checkout maps the same native module files and a plain `lsof +D` reports it under the worktree path; only cwd, txt, and open descriptors mark a holder.
...
- verification states: where the diff changes ordering, filtering or layout, the take drives the non-default states too - the other sort keys, the other filter states, collapsed and expanded chrome, every viewport the design direction names - and its report names the states it exercised; a proof that only covers the state the change was designed around is not a proof
- deployment rules: deployment-side state (env vars on a dev or preview deployment, remote config) is driver-owned - a take that needs it changed states the need in its report and stays local; a contract that explicitly grants a take deployment-env access adds two obligations: restore values raw (a value copied out of a dotenv file carries its surrounding quotes as literal characters - strip them) and list every var touched in the report
...
A report that mentions deployment env changes gets the same treatment: read the deployment's current values yourself and check them against expected - "restored and verified" is a claim, and a value restored from a dotenv file typically carries stray literal quotes.
