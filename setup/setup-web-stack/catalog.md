...
- A repo `verify` skill plus an ephemeral dev-server script - scaffold both from the installed `verify-protocol` skill's `verify-reference.md` (source: wellgent/skills, pinned beside `orchestrate`): the project-specific manual for launching, signing in as an agent, and driving the app, and the port-scoped `scripts/dev-server.sh` it rides on. The dev-loop workflow's gate calls `/verify` for any ticket with runtime surface, and the skill is where hard-won driving gotchas accumulate
